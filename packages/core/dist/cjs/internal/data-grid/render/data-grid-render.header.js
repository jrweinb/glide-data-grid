...
exports.drawHeader = exports.getActionBoundsForGroup = exports.getHeaderMenuBounds = exports.drawGroups = exports.drawGridHeaders = void 0;
...
function drawHeaderInner(ctx, x, y, width, height, c, selected, theme, isHovered, hoverAmount, spriteManager, touchMode, isRtl, isCheckboxHeader, menuBounds) {
    if (isCheckboxHeader) {
        ...
        if (c.title === data_grid_types_js_1.headerCellCheckedMarker)
            checked = true;
        if (c.title === data_grid_types_js_1.headerCellUnheckedMarker)
            checked = false;
        if (checked !== true) {
            ctx.globalAlpha = hoverAmount;
        }
        (0, draw_checkbox_js_1.drawCheckbox)(ctx, theme, checked, x, y, width, height, false, undefined, undefined, 18);
        if (checked !== true) {
            ctx.globalAlpha = 1;
        }
        return;
    ...
    ctx.fillText(c.title, drawX, y + height / 2 + (0, data_grid_lib_js_1.getMiddleCenterBias)(ctx, theme.headerFontFull));
    if (isRtl) {
        ctx.textAlign = "left";
    }
...
function drawHeader(ctx, x, y, width, height, c, selected, theme, isHovered, hasSelectedCell, hoverAmount, spriteManager, drawHeaderCallback, touchMode) {
    const isCheckboxHeader = c.title.startsWith(data_grid_types_js_1.headerCellCheckboxPrefix);
