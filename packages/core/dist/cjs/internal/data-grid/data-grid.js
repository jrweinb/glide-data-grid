...
const data_grid_sprites_js_1 = require("./data-grid-sprites.js");
const utils_js_1 = require("../../common/utils.js");
const clamp_js_1 = __importDefault(require("lodash/clamp.js"));
const range_js_1 = __importDefault(require("lodash/range.js"));
const data_grid_render_js_1 = require("./render/data-grid-render.js");
const animation_manager_js_1 = require("./animation-manager.js");
...
const use_animation_queue_js_1 = require("./use-animation-queue.js");
const support_js_1 = require("../../common/support.js");
const event_args_js_1 = require("./event-args.js");
const math_js_1 = require("../../common/math.js");
const data_grid_render_cells_js_1 = require("./render/data-grid-render.cells.js");
const data_grid_render_header_js_1 = require("./render/data-grid-render.header.js");
...
const DataGrid = (p, forwardedRef) => {
    ...
    const spriteManager = React.useMemo(() => new data_grid_sprites_js_1.SpriteManager(headerIcons, () => {
        lastArgsRef.current = undefined;
        lastDrawRef.current();
    }), [headerIcons]);
    const totalHeaderHeight = enableGroups ? groupHeaderHeight + headerHeight : headerHeight;
    const scrollingStopRef = React.useRef(-1);
    const enableFirefoxRescaling = (experimental?.enableFirefoxRescaling ?? false) && browser_detect_js_1.browserIsFirefox.value;
    const enableSafariRescaling = (experimental?.enableSafariRescaling ?? false) && browser_detect_js_1.browserIsSafari.value;
    ...
    const mappedColumns = (0, data_grid_lib_js_1.useMappedColumns)(columns, freezeColumns);
    const stickyX = fixedShadowX ? (0, data_grid_lib_js_1.getStickyWidth)(mappedColumns, dragAndDropState) : 0;
    ...
    const getBoundsForItem = React.useCallback((canvas, col, row) => {
        ...
        const result = (0, data_grid_lib_js_1.computeBounds)(col, row, width, height, groupHeaderHeight, totalHeaderHeight, cellXOffset, cellYOffset, translateX, translateY, rows, freezeColumns, freezeTrailingRows, mappedColumns, rowHeight);
        if (scale !== 1) {
            result.x *= scale;
            result.y *= scale;
            result.width *= scale;
    ...
    const getMouseArgsForPosition = React.useCallback((canvas, posX, posY, ev) => {
        ...
        const effectiveCols = (0, data_grid_lib_js_1.getEffectiveColumns)(mappedColumns, cellXOffset, width, undefined, translateX);
        let button = 0;
        let buttons = 0;
        ...
        // -1 === off right edge
        const col = (0, data_grid_lib_js_1.getColumnIndexForX)(x, effectiveCols, translateX);
        // -1: header or above
        // undefined: offbottom
        const row = (0, data_grid_lib_js_1.getRowIndexForY)(y, height, enableGroups, headerHeight, groupHeaderHeight, rows, rowHeight, cellYOffset, translateY, freezeTrailingRows);
        ...
        if (col === -1 || y < 0 || x < 0 || row === undefined || x > width || y > height) {
            ...
            if (horizontal === 0)
                innerHorizontal = col === -1 ? event_args_js_1.OutOfBoundsRegionAxis.EndPadding : event_args_js_1.OutOfBoundsRegionAxis.Center;
            if (vertical === 0)
                innerVertical = row === undefined ? event_args_js_1.OutOfBoundsRegionAxis.EndPadding : event_args_js_1.OutOfBoundsRegionAxis.Center;
            let isEdge = false;
            if (col === -1 && row === -1) {
                const b = getBoundsForItem(canvas, mappedColumns.length - 1, -1);
                (0, support_js_1.assert)(b !== undefined);
            ...
            // by design.
            const isMaybeScrollbar = (x > width && x < width + (0, utils_js_1.getScrollBarWidth)()) || (y > height && y < height + (0, utils_js_1.getScrollBarWidth)());
            result = {
                kind: event_args_js_1.outOfBoundsKind,
            ...
            (0, support_js_1.assert)(bounds !== undefined);
            ...
            if (posX - bounds.x <= edgeDetectionBuffer && previousCol >= 0) {
                ...
                (0, support_js_1.assert)(bounds !== undefined);
                result = {
                    kind: enableGroups && row === -2 ? event_args_js_1.groupHeaderKind : event_args_js_1.headerKind,
            ...
            if (fillHandle && selection.current !== undefined) {
                const fillHandleLocation = (0, data_grid_lib_js_1.rectBottomRight)(selection.current.range);
    ...
    const renderStateProvider = React.useMemo(() => new render_state_provider_js_1.RenderStateProvider(), []);
    ...
    const draw = React.useCallback(() => {
        ...
        const current = {
            ...
            headerHeight,
            groupHeaderHeight,
            disabledRows: disabledRows ?? data_grid_types_js_1.CompactSelection.empty(),
            rowHeight,
            verticalBorder,
            isResizing,
            resizeCol,
            ...
            renderStateProvider,
            renderStrategy: experimental?.renderStrategy ?? (browser_detect_js_1.browserIsSafari.value ? "double-buffer" : "single-buffer"),
            getCellRenderer,
            minimumCellWidth,
        };
        ...
        if (current.damage === undefined) {
            ...
            (0, data_grid_render_js_1.drawGrid)(current, last);
        }
        else {
            (0, data_grid_render_js_1.drawGrid)(current, undefined);
        }
    ...
    const enqueue = (0, use_animation_queue_js_1.useAnimationQueue)(damageInternal);
    enqueueRef.current = enqueue;
    const damage = React.useCallback((cells) => {
        damageInternal(new cell_set_js_1.CellSet(cells.map(x => x.cell)));
    }, [damageInternal]);
    ...
    if (cursorOverride === undefined && hCol !== undefined && hRow !== undefined && hRow > -1 && hRow < rows) {
        ...
        clickableInnerCellHovered =
            cell.kind === data_grid_types_js_1.InnerGridCellKind.NewRow ||
                (cell.kind === data_grid_types_js_1.InnerGridCellKind.Marker && cell.markerKind !== "number");
        editableBoolHovered = cell.kind === data_grid_types_js_1.GridCellKind.Boolean && (0, data_grid_types_js_1.booleanCellIsEditable)(cell);
    ...
    const groupHeaderActionForEvent = React.useCallback((group, bounds, localEventX, localEventY) => {
        ...
        if (groupDesc.actions !== undefined) {
            const boxes = (0, data_grid_render_header_js_1.getActionBoundsForGroup)(bounds, groupDesc.actions);
            for (const [i, box] of boxes.entries()) {
                if ((0, math_js_1.pointInRect)(box, localEventX + bounds.x, localEventY + box.y)) {
    ...
    const isOverHeaderMenu = React.useCallback((canvas, col, clientX, clientY) => {
        ...
        if (!isDragging && !isResizing && header.hasMenu === true && !(hoveredOnEdge ?? false)) {
            ...
            (0, support_js_1.assert)(headerBounds !== undefined);
            const menuBounds = (0, data_grid_render_header_js_1.getHeaderMenuBounds)(headerBounds.x, headerBounds.y, headerBounds.width, headerBounds.height, (0, utils_js_1.direction)(header.title) === "rtl");
    ...
    (0, utils_js_1.useEventListener)("touchstart", onMouseDownImpl, window, false);
    (0, utils_js_1.useEventListener)("mousedown", onMouseDownImpl, window, false);
    ...
    const onMouseUpImpl = React.useCallback((ev) => {
        ...
        if (args.kind === event_args_js_1.headerKind && headerBounds !== undefined) {
    ...
    (0, utils_js_1.useEventListener)("mouseup", onMouseUpImpl, window, false);
    (0, utils_js_1.useEventListener)("touchend", onMouseUpImpl, window, false);
    ...
    (0, utils_js_1.useEventListener)("click", onClickImpl, window, false);
    ...
    (0, utils_js_1.useEventListener)("contextmenu", onContextMenuImpl, eventTargetRef?.current ?? null, false);
    const onAnimationFrame = React.useCallback(values => {
        damageRegion.current = new cell_set_js_1.CellSet(values.map(x => x.item));
        hoverValues.current = values;
        lastDrawRef.current();
        damageRegion.current = undefined;
    }, []);
    const animManagerValue = React.useMemo(() => new animation_manager_js_1.AnimationManager(onAnimationFrame), [onAnimationFrame]);
    ...
    React.useLayoutEffect(() => {
        ...
        const r = getCellRenderer(cell);
        am.setHovered((r === undefined && cell.kind === data_grid_types_js_1.GridCellKind.Custom) || r?.needsHover === true ? hoveredItem : undefined);
    ...
    const onMouseMoveImpl = React.useCallback((ev) => {
        ...
        if (!(0, event_args_js_1.mouseEventArgsAreEqual)(args, hoveredRef.current)) {
            onItemHovered?.(args);
            maybeSetHoveredInfo(args.kind === event_args_js_1.outOfBoundsKind ? undefined : [args.location, [args.localEventX, args.localEventY]], true);
            hoveredRef.current = args;
        }
        else if (args.kind === "cell" || args.kind === event_args_js_1.headerKind || args.kind === event_args_js_1.groupHeaderKind) {
            ...
            if (args.kind === "cell") {
                ...
                needsHoverPosition = rendererNeeds ?? toCheck.kind === data_grid_types_js_1.GridCellKind.Custom;
            ...
            if (needsDamageCell) {
                damageInternal(new cell_set_js_1.CellSet([args.location]));
            }
        }
        const notRowMarkerCol = args.location[0] >= (firstColAccessible ? 0 : 1);
        setHoveredOnEdge(args.kind === event_args_js_1.headerKind && args.isEdge && notRowMarkerCol && allowResize === true);
    ...
    (0, utils_js_1.useEventListener)("mousemove", onMouseMoveImpl, window, true);
        ...
        if (!prevented && dragMime !== undefined && dragData !== undefined && event.dataTransfer !== null) {
            ...
            if (dragImage !== undefined && dragImageX !== undefined && dragImageY !== undefined) {
                ...
                if (row !== undefined) {
                    ...
                    (0, support_js_1.assert)(boundsForDragTarget !== undefined);
                    ...
                    if (ctx !== null) {
                        ...
                        if (row === -1) {
                            ...
                            (0, data_grid_render_header_js_1.drawHeader)(ctx, 0, 0, boundsForDragTarget.width, boundsForDragTarget.height, mappedColumns[col], false, theme, false, false, 0, spriteManager, drawHeaderCallback, false);
                        }
                        else {
                            ctx.font = theme.baseFontFull;
                            ctx.fillStyle = theme.bgCell;
                            ctx.fillRect(0, 0, offscreen.width, offscreen.height);
                            (0, data_grid_render_cells_js_1.drawCell)(ctx, getCellContent([col, row]), 0, row, false, false, 0, 0, boundsForDragTarget.width, boundsForDragTarget.height, false, theme, theme.bgCell, imageLoader, spriteManager, 1, undefined, false, 0, undefined, undefined, undefined, renderStateProvider, getCellRenderer, () => undefined);
                        }
                    }
    ...
    (0, utils_js_1.useEventListener)("dragstart", onDragStartImpl, eventTargetRef?.current ?? null, false, false);
    ...
    (0, utils_js_1.useEventListener)("dragover", onDragOverImpl, eventTargetRef?.current ?? null, false, false);
    const onDragEndImpl = React.useCallback(() => {
        activeDropTarget.current = undefined;
        onDragEnd?.();
    }, [onDragEnd]);
    (0, utils_js_1.useEventListener)("dragend", onDragEndImpl, eventTargetRef?.current ?? null, false, false);
    ...
    (0, utils_js_1.useEventListener)("drop", onDropImpl, eventTargetRef?.current ?? null, false, false);
    const onDragLeaveImpl = React.useCallback(() => {
        onDragLeave?.();
    }, [onDragLeave]);
    (0, utils_js_1.useEventListener)("dragleave", onDragLeaveImpl, eventTargetRef?.current ?? null, false, false);
    ...
    const accessibilityTree = (0, utils_js_1.useDebouncedMemo)(() => {
        if (width < 50 || experimental?.disableAccessibilityTree === true)
            return null;
        let effectiveCols = (0, data_grid_lib_js_1.getEffectiveColumns)(mappedColumns, cellXOffset, width, dragAndDropState, translateX);
        ...
        const visibleRows = (0, range_js_1.default)(cellYOffset, Math.min(rows, cellYOffset + accessibilityHeight));
        ...
        return (React.createElement("table", { key: "access-tree", role: "grid", "aria-rowcount": rows + 1, "aria-multiselectable": "true", "aria-colcount": mappedColumns.length + colOffset },
            ...
            React.createElement("tbody", { role: "rowgroup" }, visibleRows.map(row => (React.createElement("tr", { role: "row", "aria-selected": selection.rows.hasIndex(row), key: row, "aria-rowindex": row + 2 }, effectiveCols.map(c => {
                ...
                const key = (0, render_state_provider_js_1.packColRowToNumber)(col, row);
                ...
                return (React.createElement("td", { key: key, role: "gridcell", "aria-colindex": col + 1 + colOffset, "aria-selected": selected, "aria-readonly": (0, data_grid_types_js_1.isInnerOnlyCell)(cellContent) || !(0, data_grid_types_js_1.isReadWriteCell)(cellContent), id: id, "data-testid": id, onClick: () => {
    ...
    const opacityX = freezeColumns === 0 || !fixedShadowX ? 0 : cellXOffset > freezeColumns ? 1 : (0, clamp_js_1.default)(-translateX / 100, 0, 1);
    const absoluteOffsetY = -cellYOffset * 32 + translateY;
    const opacityY = !fixedShadowY ? 0 : (0, clamp_js_1.default)(-absoluteOffsetY / 100, 0, 1);
...
exports.default = React.memo(React.forwardRef(DataGrid));
